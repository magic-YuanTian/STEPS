...
from backend.structuredSmBop.smbop.eval_final.evaluation import evaluate_single
from backend.structuredSmBop.smbop.utils import ra_postproc
from backend.structuredSmBop.smbop.utils import vec_utils
from backend.structuredSmBop.smbop.utils import hashing

logger = logging.getLogger(__name__)

@Model.register("smbop_parser_structured")
class SmbopParser(Model):
    def __init__(
            ...
            decoder_timesteps=7,  # original: 9
            beam_size=30,  # original: 30
            misc_params=None,
            dropout: float = 0.1,
    ) -> None:
        super().__init__(vocab)
        self._experiment_name = experiment_name  # experiment name
        self._misc_params = misc_params  # some hyperparameters
        self.set_flags()  #
    ...
    def _create_type_tensor(self):
        ...
        # that could be the root of the AST
        self._term_ids = [
            self._type_dict[i]
            for i in [
                "Project",
        ...
        if not self.training:
            ...
            if (
                ...
                for top_beam_indices_el, is_gold_leaf_el in zip(
                    ...
                    # if not isinstance(top_beam_indices_el, list):
                    #     top_beam_indices_el = [top_beam_indices_el]
            ...
            for b in range(batch_size):
                ...
                if hash_gold_tree is not None:
                    try:
                        ...
                        spider_acc = matchScore(gold_sql, sql)
                    except Exception as e:
