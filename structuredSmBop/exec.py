# this program is used to train structured explanation ---> sql subexpression
...
def run():
    ...
    # experiment_name = "pretty-turquoise-tapir"
...
#     sh.ln("-s", f"{prefix}/experiments/{experiment_name}", f"experiments/{experiment_name}")
    ...
    # dataset_reader = SmbopSpiderDatasetReader()
    # instances = list(dataset_reader.read(file_path="train_spider.json"))
    ...
    with cntx:
        train_model(
            params=settings,
            serialization_dir=f"experiments/{experiment_name}", # The directory in which to save results and logs.
            # recover=args.recover,
            recover=False,
            force=True, # If True, we will overwrite the serialization directory if it already exists.
        )
