...
def irra_to_sql(tree, peren=True):
    # jump subquery
    if tree.name == "Subquery":
        return irra_to_sql(tree.children[0])

    if tree.name == 'Table':
        return 'FROM ' + tree.val
    elif tree.name == 'Where':
        return 'WHERE ' + irra_to_sql(tree.children[0])
    ...
    elif tree.name == "Limit":
        return (
                irra_to_sql(tree.children[1])
                + " LIMIT "
                + str(tree.children[0].val)
        )

    elif tree.name == "Groupby":
        if len(tree.children) >= 1: # should be 1
            return (
                    "GROUP BY "
                    + irra_to_sql(tree.children[0])
            )
    ...
    elif tree.name == "Having_clause":
        # if len(tree.children) > 1:
        #     if tree.children[1].name == "Groupby": # should be here
        #         return (
        #                 irra_to_sql(tree.children[1]) # group by
        #                 + " HAVING "
        #                 + irra_to_sql(tree.children[0]) # condition
        #         )
        #     else:
        #         return (
        #                 irra_to_sql(tree.children[0]) # group by
        #                 + "HAVING"
        #                 + irra_to_sql(tree.children[1]) # condition
        #         )
        # else:
        return (
                "HAVING "
        ...
        if len(tree.children) == 1:
            return "FROM " + irra_to_sql(tree.children[0])
        return (
                "FROM "
                + irra_to_sql(tree.children[1])
                + " WHERE "
...
def ra_to_irra(tree):
    ...
    flat_tree = Node("Subquery", children=[flat_tree]) # this line could be deleted
    promote(flat_tree)
